 ...
 * 04/11/2007
 */
package ucm.gaia.jcolibri.method.retrieve.DiverseByMedianRetrieval;
...
public class ExpertClerkMedianScoring
{
     ...
     * @param cases to retrieve from
     * @param simConfig is the nn configuration
	...
	for(Attribute at: AttributeUtils.getAttributes(res))
	{
		...
		if(at.getType().equals(Integer.class))
		    AttributeUtils.setValue(at, res, ((Number)(sum/(double)cases.size())).intValue() );
		else if(at.getType().equals(Double.class))
		    AttributeUtils.setValue(at, res, (sum/(double)cases.size()) );
		else if(at.getType().equals(Float.class))
		    AttributeUtils.setValue(at, res, ((Number)(sum/(double)cases.size())).floatValue() );
		else if(at.getType().equals(Long.class))
		    AttributeUtils.setValue(at, res, ((Number)(sum/(double)cases.size())).longValue() );
		else if(at.getType().equals(Short.class))
		    AttributeUtils.setValue(at, res, ((Number)(sum/(double)cases.size())).shortValue() );
	    }
	    else
