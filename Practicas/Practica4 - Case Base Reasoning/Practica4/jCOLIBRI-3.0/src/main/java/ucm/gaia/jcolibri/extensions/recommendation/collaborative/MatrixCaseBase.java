 ...
 * These kind of recommendations are based in the past experieces of other users.<br>
 ...
 * result stores the rating value for the item (the value that the user assigns to an item after evaluating it).<br>
 * This way, this case base implementation stores cases as a table:<br>
 * <table border="1" cellspacing="0" cellpadding="2" width="90">
 *  <tr><td></td><td>Item1</td><td>Item2</td><td>Item3</td><td>Item4</td><td>Item5</td><td>...</td><td>ItemM</td></tr>
 *  <tr><td>User1</td><td></td><td>rating12</td><td></td><td>rating14</td><td></td><td></td><td></td></tr>
 *  <tr><td>User2</td><td>rating21</td><td></td><td>rating23</td><td></td><td></td><td></td><td>rating2N</td></tr>
 *  <tr><td>User3</td><td></td><td></td><td>rating33</td><td>rating34</td><td></td><td></td><td>rating3N</td></tr>
 *  <tr><td>...</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
 *  <tr><td>UserN</td><td></td><td>ratingN2</td><td></td><td></td><td>ratingN5</td><td></td><td>ratingNN</td></tr>
 * </table>
 * The values of the users column and the items row are the ids of the description and solution parts of the case.
 * These ids must be integer values.
 * The ratings are obtained from an attribute of the result part.
 * <br>
 ...
 * <li>Normalize ratings and compute a prediction from a weighted combination of selected neighbors' ratings.
 * </ol>
 * There are several ways to compute first step. And it must be implemented by subclassing this class as in PearsonMatrixCaseBase.
 * The subclasses must implement the abstract methods defined here that return the similarity among neighbors.
 * The similarity among users is stored through SimilarityTuple objects.
 * <p>
 * To allow an efficient comparison, ratings must be sorted by neighbors. In the above table this means that cases
 * are organized by rows. And that is done internally in this class through the
 * organizeByDescriptionId() method. Then, the ratings of each user can be accessed through the getRatingTuples()
 * method. A RatingTuple object stores the id of the item and its rating (id of the solution and id of the result).
 * <p>
 * There are also 2 internal classes (CommonRatingTuple and CommonRatingsIterator) that allow to obtain efficiently the common 
 * ratings of two users. This will be used by subclasses when computing the neighbors similarity. The code of these classes
 * is an adaptation of the one developed by Jerome Kelleher and Derek Bridge for the Collaborative Movie Recommender project at
...
public abstract class MatrixCaseBase implements CBRCaseBase
{
     ...
     * @param id1 of the first neighbor
     * @param id2 of the second neighbor.
     ...
     * Organizes ratings by the description id of the cases and calls the computeSimilarities() abstract method.
     */
    public void init(Connector connector) {
	this.connector = connector;
	ProgressController.init(MatrixCaseBase.class,"Organizing Collaborative Case Base", ProgressController.UNKNOWN_STEPS);
	LogManager.getLogger().info("Loading cases from connector");
     ...
     * @param id of the description object
     ...
     * @param id of the solution object
    ...
    {
	LogManager.getLogger().info("Organizing cases");
	try
	{
     ...
     * @version 1.0
     */
    public class RatingTuple implements Comparable
    {
	int solutionId;
	double rating;
     ...
     *
     */
    public class SimilarTuple implements Comparable
    {
	int similarId;
	double similarity;
    ...
    // Project implemented by Jerome Kelleher at University College Cork (Ireland)   //
    ///////////////////////////////////////////////////////////////////////////////////
	 ...
	 * Constructor
	 * @param id1 of user1
	 * @param rating1 of user1
	 * @param id2 of user 2
	 * @param rating2 of user2
	 * @param ratedId is the id of the rated object.
	 */
     ...
     *
     */
    class CommonRatingsIterator implements Iterator
    {
	int idA;
	int idB;
	...
	public CommonRatingsIterator(int idA, int idB, Collection<RatingTuple> rtsA, Collection<RatingTuple> rtsB)
	{
	    ...
	    this.rtsB = rtsB;
	      ...
	      {  
		 rka = (RatingTuple) iterA.next();
	         rkb = (RatingTuple) iterB.next();
	         moreRatings = true; // MUST be before the first call to getNextMatch!
	   ...
	   /**
	    * Return true iff there are more common ratings
	    * in this CommonRatingsIterator.
	    *
	   ...
	   /**
	    * Return the next match in this CommonRatingsIterator.
	    * This is in the form of a CommonRatingTuple. This allows both
	    * ratings concerned be returned to the client code, thus
	    * saving a scan through the entire ratings collection.
	    *
	    * @return a RatingTuple of the next two common ratings.
	    * @throws NoSuchElementException if no more common ratings
	    * exist.
	    */
	   public Object next()
	   ...
	   /**
	    * Return the next common rating between the two users.
	    * Return null if  no matches are left.
	    *
	    * @return a RatingTuple of the next match in the two user
	    * collections of ratings; null if no matches exist.
	    */
