...
{ 
     ...
     * Partially replaces current query with the description of the selected case.
     */
    public static void partialMoreLikeThis(CBRQuery query, CBRCase selectedCase, List<CBRCase> proposedCases)
    {
	...
	for(Attribute at: AttributeUtils.getAttributes(selectedCase.getDescription()))
	{
		...
		{    
		    ...
		    copyAttribute = !selectedValue.equals(value);
		if(!copyAttribute)
		    break;
	    }
	    if(copyAttribute)
