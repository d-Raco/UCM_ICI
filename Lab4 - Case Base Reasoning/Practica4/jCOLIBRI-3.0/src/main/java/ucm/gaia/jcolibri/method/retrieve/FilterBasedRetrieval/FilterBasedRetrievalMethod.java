/**
 * FilterBasedRetrievalMethod.java
 * jCOLIBRI2 framework. 
 ...
 * 28/10/2007
 */
package ucm.gaia.jcolibri.method.retrieve.FilterBasedRetrieval;
...
public class FilterBasedRetrievalMethod
{
	...
	try
	{
	    for(Attribute att : AttributeUtils.getAttributes(cc))
	    {
	        ...
	        else if(att.equals(cc.getIdAttribute()))
	                continue;
	        else
	        {
	    		...
	    		if(predicate == null)
	    		    continue;
	    		if(!predicate.compute(att.getValue(cc), att.getValue(qc)))
	    		    return false;
	        }
	    }
	    return true;
	...
	{
	    LogManager.getLogger(FilterBasedRetrievalMethod.class).error(e);
	    return false; 
	}
    }
}
